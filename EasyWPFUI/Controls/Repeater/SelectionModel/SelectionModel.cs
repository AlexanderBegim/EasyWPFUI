...
namespace EasyWPFUI.Controls
{
    public class SelectedItemInfo
    {
        ...
        public WeakReference<SelectionNode> Node { get; set; }
    ...
    public class SelectionModel : INotifyPropertyChanged
    {
            ...
            set
            {
                if(m_singleSelect != !!value)
                {
                    ...
                    if (value && selectedIndices != null && selectedIndices.Count > 1)
                    {
                        ...
                        // one selected item.
                        IndexPath firstSelectionIndexPath = selectedIndices[0];
                        ClearSelection(true /* resetAnchor */, false /*raiseSelectionChanged */);
        ...
        public IndexPath AnchorIndex
        {
            get
            {
                ...
                if (m_rootNode.AnchorIndex >= 0)
                {
                    ...
                    SelectionNode current = m_rootNode;
        ...
        public IndexPath SelectedIndex
        {
            ...
            set
            {
                bool? isSelected = IsSelectedAt(value);
        ...
        public bool? IsSelected(int index)
        {
            ...
            bool? isSelected = m_rootNode.IsSelectedWithPartial(index);
            return isSelected;
        }
        ...
        public bool? IsSelected(int groupIndex, int itemIndex)
        {
            ...
            SelectionNode childNode = m_rootNode.GetAt(groupIndex, false /*realizeChild*/);
            if (childNode != null)
            {
        ...
        public bool? IsSelectedAt(IndexPath index)
        {
            ...
            SelectionNode node = m_rootNode;
            for (int i = 0; i < path.GetSize() - 1; i++)
            {
                int childIndex = path.GetAt(i);
        ...
        public object ResolvePath(object data, IndexPath dataIndexPath)
        {
        ...
        public SelectionNode SharedLeafNode()
        {
            return m_leafNode;
        }
        ...
        private void SelectImpl(int index, bool select)
        {
            if (m_rootNode.IsSelected(index) != select)
            {
                ...
                bool selected = m_rootNode.Select(index, select);
                if (selected)
                {
                    AnchorIndex = new IndexPath(index);
                }
        ...
        private void SelectWithGroupImpl(int groupIndex, int itemIndex, bool select)
        {
            ...
            SelectionNode childNode = m_rootNode.GetAt(groupIndex, true /* realize */);
            bool selected = childNode.Select(itemIndex, select);
            if (selected)
            {
                AnchorIndex = new IndexPath(groupIndex, itemIndex);
            }
        ...
        private void SelectWithPathImpl(IndexPath index, bool select, bool raiseSelectionChanged)
        {
            ...
            if (m_singleSelect)
            {
                if (SelectedIndex is IndexPath selectedIndex)
                {
        ...
        private void SelectRangeFromAnchorWithGroupImpl(int groupIndex, int itemIndex, bool select)
        {
            ...
            // Make sure start > end
            if (startGroupIndex > groupIndex ||
                (startGroupIndex == groupIndex && startItemIndex > itemIndex))
            {
            ...
            bool selected = false;
            for (int groupIdx = startGroupIndex; groupIdx <= groupIndex; groupIdx++)
            {
                SelectionNode groupNode = m_rootNode.GetAt(groupIdx, true /* realizeChild */);
                int startIndex = groupIdx == startGroupIndex ? startItemIndex : 0;
                int endIndex = groupIdx == groupIndex ? itemIndex : groupNode.DataCount - 1;
        ...
        {
            IndexPath winrtStart = start;
            IndexPath winrtEnd = end;

            // Make sure start <= end 
            if (winrtEnd.CompareTo(winrtStart) == -1)
            {
                IndexPath temp = winrtStart;
                winrtStart = winrtEnd;
                winrtEnd = temp;
            }
